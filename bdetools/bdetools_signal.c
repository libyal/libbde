...
void (*bdetools_signal_signal_handler)( bdetools_signal_t ) = NULL;
             ...
             unsigned long signal )
{
	...
	switch( signal )
	{
		...
		case CTRL_C_EVENT:
			if( bdetools_signal_signal_handler != NULL )
			{
				bdetools_signal_signal_handler(
				 signal );
			}
			return( TRUE );
...
{
	static char *function = "bdetools_signal_attach";
...
#if defined( _MSC_VER )
	bdetools_signal_initialize_memory_debug();
#endif
     ...
     libcerror_error_t **error )
{
	...
	bdetools_signal_signal_handler = NULL;
