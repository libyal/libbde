...
LIBRARY_TESTS="aes_ccm_encrypted_key error external_key io_handle key key_protector metadata metadata_entry notify password_keep sector_data stretch_key volume_master_key";
...
run_test_with_input()
{
	...
	run_test_on_input_directory "libbde" "${TEST_DESCRIPTION}" "default" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "input" "${INPUT_GLOB}";
	local RESULT=$?;
