...
LIBRARY_TESTS="aes_ccm_encrypted_key error external_key io_handle key key_protector metadata metadata_block_header metadata_entry metadata_header notify password_keep sector_data stretch_key volume_master_key";
...
run_test()
{
	...
	local TEST_EXECUTABLE="./bde_test_${TEST_NAME}";
...
run_test_with_input()
{
	...
	local TEST_PROFILE_DIRECTORY=$(get_test_profile_directory "input" "libbde");
