/*
 * Library stretch_key type testing program
 *
...
#include "../libbde/libbde_stretch_key.h"

#if defined( __GNUC__ )

/* Tests the libbde_stretch_key_initialize function
 * Returns 1 if successful or 0 if not
 */
int bde_test_stretch_key_initialize(
     void )
{
	...
	/* Test regular cases
	 */
	result = libbde_stretch_key_initialize(
	          &stretch_key,
	          &error );
	...
	stretch_key = (libbde_stretch_key_t *) 0x12345678UL;
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libbde_stretch_key_initialize with malloc failing
		 */
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libbde_stretch_key_initialize with memset failing
		 */
...
/* Tests the libbde_stretch_key_free function
 * Returns 1 if successful or 0 if not
 */
int bde_test_stretch_key_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
#endif
{
	...
	BDE_TEST_RUN(
	 "libbde_stretch_key_initialize",
	 bde_test_stretch_key_initialize );

	BDE_TEST_RUN(
	 "libbde_stretch_key_free",
	 bde_test_stretch_key_free );

	/* TODO: add tests for libbde_stretch_key_read */
