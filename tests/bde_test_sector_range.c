...
#include "../libbde/libbde_sector_range.h"

#if defined( __GNUC__ ) && !defined( LIBBDE_DLL_IMPORT )

/* Tests the libbde_sector_range_initialize function
 * Returns 1 if successful or 0 if not
 */
int bde_test_sector_range_initialize(
     void )
{
	...
	/* Test regular cases
	 */
	result = libbde_sector_range_initialize(
	          &sector_range,
	          &error );
	...
	result = libbde_sector_range_free(
	          &sector_range,
	          &error );
	...
	sector_range = (libbde_sector_range_t *) 0x12345678UL;
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libbde_sector_range_initialize with malloc failing
		 */
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libbde_sector_range_initialize with memset failing
		 */
...
/* Tests the libbde_sector_range_free function
 * Returns 1 if successful or 0 if not
 */
int bde_test_sector_range_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
#endif
{
	...
	BDE_TEST_RUN(
	 "libbde_sector_range_initialize",
	 bde_test_sector_range_initialize );

	BDE_TEST_RUN(
	 "libbde_sector_range_free",
	 bde_test_sector_range_free );
