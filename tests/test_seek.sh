...
{ 
	...
	${TEST_RUNNER} ${TMPDIR} ${TEST_SEEK} ${OPTIONS} ${INPUT_FILE};
...
do
	...
	do
		...
		do
			...
			echo "Testing seek with option: ${OPTION_SET} and input: ${INPUT_FILE}";

			if test_seek "${TEST_SET}" "${TEST_FILE}" "${OPTION_SET}";
			then
				RESULT=${EXIT_SUCCESS};
			else
				RESULT=${EXIT_FAILURE};
		...
		then
			echo "Testing seek with input: ${INPUT_FILE}";

			if test_seek "${TEST_SET}" "${TEST_FILE}" "";
			then
				RESULT=${EXIT_SUCCESS};
			else
				RESULT=${EXIT_FAILURE};
