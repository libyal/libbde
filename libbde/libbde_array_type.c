 ...
 * The entry_compare_function should return LIBBDE_ARRAY_COMPARE_LESS,
 * LIBBDE_ARRAY_COMPARE_EQUAL, LIBBDE_ARRAY_COMPARE_GREATER if successful or -1 on error
 *
 * Duplicate entries are allowed by default and inserted after the last duplicate entry.
 * Only allowing unique entries can be enforced by setting the flag LIBBDE_ARRAY_INSERT_FLAG_UNIQUE_ENTRIES
 *
     ...
     liberror_error_t **error )
{
	...
	if( array->entries != NULL )
	{
		     ...
		     entry_iterator++ )
		{
			...
			else if( result == LIBBDE_ARRAY_COMPARE_EQUAL )
			{
				if( ( insert_flags & LIBBDE_ARRAY_INSERT_FLAG_UNIQUE_ENTRIES ) != 0 )
				{
					return( 0 );
			...
			else if( result == LIBBDE_ARRAY_COMPARE_LESS )
			{
				break;
			}
	...
	if( ( array->entries != NULL )
	 && ( result == LIBBDE_ARRAY_COMPARE_LESS ) )
	{
