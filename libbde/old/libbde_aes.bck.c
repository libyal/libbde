...
/* TODO replace Y with more descriptive name */

#define libbde_aes_calculate_forward_round( round_keys, values_32bit, Y ) \
{ \
	values_32bit[ 0 ] = *round_keys++ \
	                  ^ libbde_aes_forward_table0[ ( Y[ 0 ] ) & 0xff ] \
	                  ^ libbde_aes_forward_table1[ ( Y[ 1 ] >>  8 ) & 0xff ] \
	                  ^ libbde_aes_forward_table2[ ( Y[ 2 ] >> 16 ) & 0xff ] \
	                  ^ libbde_aes_forward_table3[ ( Y[ 3 ] >> 24 ) & 0xff ]; \
\
	values_32bit[ 1 ] = *round_keys++ \
	                  ^ libbde_aes_forward_table0[ ( Y[ 1 ] ) & 0xff ] \
	                  ^ libbde_aes_forward_table1[ ( Y[ 2 ] >>  8 ) & 0xff ] \
	                  ^ libbde_aes_forward_table2[ ( Y[ 3 ] >> 16 ) & 0xff ] \
	                  ^ libbde_aes_forward_table3[ ( Y[ 0 ] >> 24 ) & 0xff ]; \
\
	values_32bit[ 2 ] = *round_keys++ \
	                  ^ libbde_aes_forward_table0[ ( Y[ 2 ] ) & 0xff ] \
	                  ^ libbde_aes_forward_table1[ ( Y[ 3 ] >>  8 ) & 0xff ] \
	                  ^ libbde_aes_forward_table2[ ( Y[ 0 ] >> 16 ) & 0xff ] \
	                  ^ libbde_aes_forward_table3[ ( Y[ 1 ] >> 24 ) & 0xff ]; \
\
	values_32bit[ 3 ] = *round_keys++ \
	                  ^ libbde_aes_forward_table0[ ( Y[ 3 ] ) & 0xff ] \
	                  ^ libbde_aes_forward_table1[ ( Y[ 0 ] >>  8 ) & 0xff ] \
	                  ^ libbde_aes_forward_table2[ ( Y[ 1 ] >> 16 ) & 0xff ] \
	                  ^ libbde_aes_forward_table3[ ( Y[ 2 ] >> 24 ) & 0xff ]; \
}

/* TODO replace Y with more descriptive name */

#define libbde_aes_calculate_reverse_round( round_keys, values_32bit, Y ) \
{ \
	values_32bit[ 0 ] = *round_keys++ \
	                  ^ libbde_aes_reverse_table0[ ( Y[ 0 ] ) & 0xff ] \
	                  ^ libbde_aes_reverse_table1[ ( Y[ 3 ] >>  8 ) & 0xff ] \
	                  ^ libbde_aes_reverse_table2[ ( Y[ 2 ] >> 16 ) & 0xff ] \
	                  ^ libbde_aes_reverse_table3[ ( Y[ 1 ] >> 24 ) & 0xff ]; \
\
	values_32bit[ 1 ] = *round_keys++ \
	                  ^ libbde_aes_reverse_table0[ ( Y[ 1 ] ) & 0xff ] \
	                  ^ libbde_aes_reverse_table1[ ( Y[ 0 ] >>  8 ) & 0xff ] \
	                  ^ libbde_aes_reverse_table2[ ( Y[ 3 ] >> 16 ) & 0xff ] \
	                  ^ libbde_aes_reverse_table3[ ( Y[ 2 ] >> 24 ) & 0xff ]; \
\
	values_32bit[ 2 ] = *round_keys++ \
	                  ^ libbde_aes_reverse_table0[ ( Y[ 2 ] ) & 0xff ] \
	                  ^ libbde_aes_reverse_table1[ ( Y[ 1 ] >>  8 ) & 0xff ] \
	                  ^ libbde_aes_reverse_table2[ ( Y[ 0 ] >> 16 ) & 0xff ] \
	                  ^ libbde_aes_reverse_table3[ ( Y[ 3 ] >> 24 ) & 0xff ]; \
\
	values_32bit[ 3 ] = *round_keys++ \
	                  ^ libbde_aes_reverse_table0[ ( Y[ 3 ] ) & 0xff ] \
	                  ^ libbde_aes_reverse_table1[ ( Y[ 2 ] >>  8 ) & 0xff ] \
	                  ^ libbde_aes_reverse_table2[ ( Y[ 1 ] >> 16 ) & 0xff ] \
	                  ^ libbde_aes_reverse_table3[ ( Y[ 0 ] >> 24 ) & 0xff ]; \
}
...
/* TODO */
	     ...
	     round_key_index-- )
	{
		for( byte_index = 0;
		     ...
		     byte_index++ )
		{
			byte_value1 = (uint8_t) *SK;
			byte_value2 = (uint8_t) ( *SK >> 8 );
			byte_value3 = (uint8_t) ( *SK >> 16 );
			byte_value4 = (uint8_t) ( *SK >> 24 );
			...
			byte_value4 = libbde_aes_forward_substitution_box[ byte_value4 ];

			*round_keys++ = libbde_aes_reverse_table0[ byte_value1 ]
			              ^ libbde_aes_reverse_table1[ byte_value2 ]
			              ^ libbde_aes_reverse_table2[ byte_value3 ]
			              ^ libbde_aes_reverse_table3[ byte_value4 ];
...
/* TODO */
	...
	if( bit_size == 128 )
	{
		...
		for( round_constant_index = 0;
		     ...
		     round_constant_index++ )
		{
			byte_value1 = (uint8_t) ( round_keys[ 3 ] >> 8 );
			byte_value2 = (uint8_t) ( round_keys[ 3 ] >> 16 );
			byte_value3 = (uint8_t) ( round_keys[ 3 ] >> 24 );
			byte_value4 = (uint8_t) round_keys[ 3 ];
			...
			round_key_value   = byte_value4;
			round_key_value <<= 8;
			round_key_value   = byte_value3;
			round_key_value <<= 8;
			round_key_value   = byte_value2;
			round_key_value <<= 8;
			round_key_value   = byte_value1;

			round_keys[ 4 ]  = libbde_aes_round_constants[ round_constant_index ];
			round_keys[ 4 ] ^= round_keys[ 0 ] ^ round_key_value;
	...
	else if( bit_size == 192 )
	{
		...
		for( round_constant_index = 0;
		     ...
		     round_constant_index++ )
		{
			byte_value1 = (uint8_t) ( round_keys[ 5 ] >> 8 );
			byte_value2 = (uint8_t) ( round_keys[ 5 ] >> 16 );
			byte_value3 = (uint8_t) ( round_keys[ 5 ] >> 24 );
			byte_value4 = (uint8_t) round_keys[ 5 ];
			...
			round_keys[ 6 ] ^= round_keys[ 0 ] ^ round_key_value;
	...
	else if( bit_size == 256 )
	{
		...
		for( round_constant_index = 0;
		     ...
		     round_constant_index++ )
		{
			byte_value1 = (uint8_t) ( round_keys[ 7 ] >> 8 );
			byte_value2 = (uint8_t) ( round_keys[ 7 ] >> 16 );
			byte_value3 = (uint8_t) ( round_keys[ 7 ] >> 24 );
			byte_value4 = (uint8_t) round_keys[ 7 ];
			...
			round_keys[ 8 ] ^= round_keys[ 0 ] ^ round_key_value;

			round_keys[ 9 ]  = round_keys[ 1 ] ^ round_keys[ 8 ];
			round_keys[ 10 ] = round_keys[ 2 ] ^ round_keys[ 9 ];
			round_keys[ 11 ] = round_keys[ 3 ] ^ round_keys[ 10 ];

			byte_value1 = (uint8_t) round_keys[ 11 ];
			byte_value2 = (uint8_t) ( round_keys[ 11 ] >> 8 );
			byte_value3 = (uint8_t) ( round_keys[ 11 ] >> 16 );
			byte_value4 = (uint8_t) ( round_keys[ 11 ] >> 24 );
			...
			round_keys[ 12 ] = round_keys[ 4 ] ^ round_key_value;
...
/* TODO refactor */
	...
	if( mode == LIBBDE_AES_CRYPT_MODE_ENCRYPT )
	{
		...
		values_32bit[ 0 ]  = *round_keys++;
		values_32bit[ 0 ] ^= (uint32_t) libbde_aes_forward_substitution_box[ Y[ 0 ] & 0xff ];
		values_32bit[ 0 ] ^= ( (uint32_t) libbde_aes_forward_substitution_box[ ( Y[ 1 ] >>  8 ) & 0xff ] ) <<  8;
		values_32bit[ 0 ] ^= ( (uint32_t) libbde_aes_forward_substitution_box[ ( Y[ 2 ] >> 16 ) & 0xff ] ) << 16;
		values_32bit[ 0 ] ^= ( (uint32_t) libbde_aes_forward_substitution_box[ ( Y[ 3 ] >> 24 ) & 0xff ] ) << 24;

		values_32bit[ 1 ]  = *round_keys++;
		values_32bit[ 1 ] ^= (uint32_t) libbde_aes_forward_substitution_box[ Y[ 1 ] & 0xff ];
		values_32bit[ 1 ] ^= ( (uint32_t) libbde_aes_forward_substitution_box[ ( Y[ 2 ] >>  8 ) & 0xff ] ) <<  8;
		values_32bit[ 1 ] ^= ( (uint32_t) libbde_aes_forward_substitution_box[ ( Y[ 3 ] >> 16 ) & 0xff ] ) << 16;
		values_32bit[ 1 ] ^= ( (uint32_t) libbde_aes_forward_substitution_box[ ( Y[ 0 ] >> 24 ) & 0xff ] ) << 24;

		values_32bit[ 2 ]  = *round_keys++;
		values_32bit[ 2 ] ^= (uint32_t) libbde_aes_forward_substitution_box[ Y[ 2 ] & 0xff ];
		values_32bit[ 2 ] ^= ( (uint32_t) libbde_aes_forward_substitution_box[ ( Y[ 3 ] >>  8 ) & 0xff ] ) <<  8;
		values_32bit[ 2 ] ^= ( (uint32_t) libbde_aes_forward_substitution_box[ ( Y[ 0 ] >> 16 ) & 0xff ] ) << 16;
		values_32bit[ 2 ] ^= ( (uint32_t) libbde_aes_forward_substitution_box[ ( Y[ 1 ] >> 24 ) & 0xff ] ) << 24;

		values_32bit[ 3 ]  = *round_keys++;
		values_32bit[ 3 ] ^= (uint32_t) libbde_aes_forward_substitution_box[ Y[ 3 ] & 0xff ];
		values_32bit[ 3 ] ^= ( (uint32_t) libbde_aes_forward_substitution_box[ ( Y[ 0 ] >>  8 ) & 0xff ] ) <<  8;
		values_32bit[ 3 ] ^= ( (uint32_t) libbde_aes_forward_substitution_box[ ( Y[ 1 ] >> 16 ) & 0xff ] ) << 16;
		values_32bit[ 3 ] ^= ( (uint32_t) libbde_aes_forward_substitution_box[ ( Y[ 2 ] >> 24 ) & 0xff ] ) << 24;
	}
	else
	{
		for( round_key_index = ( context->number_of_round_keys / 2 );
		...
		values_32bit[ 0 ]  = *round_keys++;
		values_32bit[ 0 ] ^= (uint32_t) libbde_aes_reverse_substitution_box[ Y[ 0 ] & 0xff ];
                values_32bit[ 0 ] ^= ( (uint32_t) libbde_aes_reverse_substitution_box[ ( Y[ 3 ] >>  8 ) & 0xff ] ) <<  8;
                values_32bit[ 0 ] ^= ( (uint32_t) libbde_aes_reverse_substitution_box[ ( Y[ 2 ] >> 16 ) & 0xff ] ) << 16;
                values_32bit[ 0 ] ^= ( (uint32_t) libbde_aes_reverse_substitution_box[ ( Y[ 1 ] >> 24 ) & 0xff ] ) << 24;

		values_32bit[ 1 ]  = *round_keys++;
		values_32bit[ 1 ] ^= (uint32_t) libbde_aes_reverse_substitution_box[ Y[ 1 ] & 0xff ];
		values_32bit[ 1 ] ^= ( (uint32_t) libbde_aes_reverse_substitution_box[ ( Y[ 0 ] >>  8 ) & 0xff ] ) <<  8;
		values_32bit[ 1 ] ^= ( (uint32_t) libbde_aes_reverse_substitution_box[ ( Y[ 3 ] >> 16 ) & 0xff ] ) << 16;
		values_32bit[ 1 ] ^= ( (uint32_t) libbde_aes_reverse_substitution_box[ ( Y[ 2 ] >> 24 ) & 0xff ] ) << 24;

		values_32bit[ 2 ]  = *round_keys++;
		values_32bit[ 2 ] ^= (uint32_t) libbde_aes_reverse_substitution_box[ Y[ 2 ] & 0xff ];
		values_32bit[ 2 ] ^= ( (uint32_t) libbde_aes_reverse_substitution_box[ ( Y[ 1 ] >>  8 ) & 0xff ] ) <<  8;
		values_32bit[ 2 ] ^= ( (uint32_t) libbde_aes_reverse_substitution_box[ ( Y[ 0 ] >> 16 ) & 0xff ] ) << 16;
		values_32bit[ 2 ] ^= ( (uint32_t) libbde_aes_reverse_substitution_box[ ( Y[ 3 ] >> 24 ) & 0xff ] ) << 24;

		values_32bit[ 3 ]  = *round_keys++;
		values_32bit[ 3 ] ^= (uint32_t) libbde_aes_reverse_substitution_box[ Y[ 3 ] & 0xff ];
		values_32bit[ 3 ] ^= ( (uint32_t) libbde_aes_reverse_substitution_box[ ( Y[ 2 ] >>  8 ) & 0xff ] ) <<  8;
		values_32bit[ 3 ] ^= ( (uint32_t) libbde_aes_reverse_substitution_box[ ( Y[ 1 ] >> 16 ) & 0xff ] ) << 16;
		values_32bit[ 3 ] ^= ( (uint32_t) libbde_aes_reverse_substitution_box[ ( Y[ 0 ] >> 24 ) & 0xff ] ) << 24;
	}
