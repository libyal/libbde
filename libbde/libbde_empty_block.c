 ...
 */
int libbde_empty_block_test(
     const uint8_t *block_buffer,
     size_t block_size,
     liberror_error_t **error )
{
	libbde_aligned_t *aligned_block_iterator = NULL;
	libbde_aligned_t *aligned_block_start    = NULL;
	uint8_t *block_iterator                  = NULL;
	uint8_t *block_start                     = NULL;
	static char *function                    = "libbde_empty_block_test";
	 ...
	 */
	if( block_size > ( 2 * sizeof( libbde_aligned_t ) ) )
	{
		/* Align the block start
		 */
		while( ( (intptr_t) block_start % sizeof( libbde_aligned_t ) ) != 0 )
		{
		 ...
		 */
		while( ( (intptr_t) block_iterator % sizeof( libbde_aligned_t ) ) != 0 )
		{
			if( *block_start != *block_iterator )
			{
				return( 0 );
		...
		aligned_block_start    = (libbde_aligned_t *) block_start;
		aligned_block_iterator = (libbde_aligned_t *) block_iterator;

		while( block_size > sizeof( libbde_aligned_t ) )
		{
			...
			block_size             -= sizeof( libbde_aligned_t );
		}
